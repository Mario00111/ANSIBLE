#!/bin/bash
####################################################
# Script de deploiement automatique des JNDI
####################################################
clear
if [ "$1" == "" ] ; then
	echo "."
	echo "."
	echo Usage : createJNDI.sh nom_du_fichier_properties
	echo Exemple : createJNDI.sh jndi.properties
	exit
fi
...
export BEA_HOME=/SOFT/oracle/wls_122130
...
${WL_HOME}/common/bin/wlst.sh createJNDI.py ${FILE_PROP}
if [ "$?" -ne "0" ]; then
	echo "."
	echo "."
