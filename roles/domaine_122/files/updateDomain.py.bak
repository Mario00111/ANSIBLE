	  ...
	  try:
		...
		while stop==false:
		  ...
		  try:
			...
			print "\t\tCreation de l'utilisateur "+username
			...
			stop=true
		print "\n\t > " + str(nb-1) +" utilisateurs crees dans le domaine.\n\n"
	  except:
		print "Unexpected error:", sys.exc_info()[0]
		print "Impossible de creer les utilisateurs"

def createJmsServer(jmsServerName,jmsTargetName):
	print "\tCreation du serveur JMS ",jmsServerName
	root=getMBean('/')
	...
	if jmsServer == None:
		...
		print "Le serveur JMS ",jmsServerName," existe deja"
	return jmsServer
...
def deployApplication(app_name, app_path, _targets):
	print "Deploiement de l'application "+app_name+" "+app_path+" sur "+_targets
	try:
...
print "*"," "*76,"*"
s = "2eme partie, mise a jour du domaine."
s = s.center(76)
print "*",s,"*"
print "*"," "*76,"*"
print "*"," "*76,"*"
...
try:
	...
	print "Creation des serveurs JMS"
	jmsServerNames=[]
	id=1
	stop=false
	while stop==false:
	...
	print "Creation des modules JMS"
	id=1
	stop=false
	while stop==false:
		try:	
			...
			print "\tCreation du sous-module JMS "+subModuleName
			cd('/SystemResources/'+moduleName)
			...
			print "Creation des connection factories"
			cfId=1
			stopCF=false
			while stopCF==false:
				try:	
					...
					print "\tCreation de la connection factory ",cfName
			...
			print "Creation des distributed Topics"
			topicId=1
			stopTopic=false
			while stopTopic==false:
				try:
					...
					print "\tCreation du topic ",topicName
	...
	clusters=root.getClusters()
	allServers=root.getServers()
	
	if len(clusters) > 0:
		cd('/SelfTuning/'+domainName+'/WorkManagers/weblogic.wsee.mdb.DispatchPolicy')
		
		targets=jarray.array([], ObjectName)
		# NOUVELLE LIGNE
		#targets.append(ObjectName('com.bea:Name='+adminServerName+',Type=Server'))
		for cluster in clusters:
			targets.append(ObjectName('com.bea:Name='+cluster.getName()+',Type=Cluster'))
		set('Targets',targets)

	else:
		if len(allServers) > 0:
			cd('/SelfTuning/'+domainName+'/WorkManagers/weblogic.wsee.mdb.DispatchPolicy')
			cmo.addTarget(getMBean("/Servers/"+adminServerName))
			for aServer in allServers:
				cmo.addTarget(getMBean("/Servers/"+aServer.getName()))
	...
	if ("WLP"==_domainType):
		...
		removeRDBMSSecurityStore()
		
		removeRDBMSSecurityStoreJMSResources()

		print "Configuration de l'identity asserter specifique..."

		cd('/SecurityConfiguration/'+domainName+'/Realms/myrealm')
		cmo.createAuthenticationProvider('CNAMIdentityAsserter', 'fr.cnamts.security.providers.CnamtsIdentityAsserter')

		cd('/SecurityConfiguration/'+domainName+'/Realms/myrealm/AuthenticationProviders/CNAMIdentityAsserter')
		### cmo.setControlFlag('OPTIONAL')
		### set('ActiveTypes',jarray.array([String('SM_USER')], String))
	

		print "Configuration de l'identity asserter terminee."

		### Sauvegarde des modifications
		save()
		activate(block="true")

	print "Arret du serveur d'administration."
