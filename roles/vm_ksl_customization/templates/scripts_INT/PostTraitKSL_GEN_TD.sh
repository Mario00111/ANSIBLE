...
#
#-------------------------------------------------------------------------------
#	Shell PostTraitKSL_GEN_TD.sh appelé par KSL
#
#
#	yann kloniecki
...
TEMP="$(basename "${2}")"
LOG_FILE="${LOG_BASE}/t${ENV}n${ENV}/cpr/${TEMP%.*}.GEN_TD.log"
...
then
	...
	then
		...
		then
		  ...
		  REP=${AFP_FILE:0:6}         #// NOUVEAU code fonctionne
      #typeset -l6 REP=${AFP_FILE} // AVANT marche plus
			...
			then
				...
				then
				  ...
				  echo "Création du REPERTOIRE OK PB  +++++  ${GED_DIR}"
				fi
			fi
