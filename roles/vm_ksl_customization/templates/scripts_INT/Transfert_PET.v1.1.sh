...
IFS_s=${IFS}; IFS="/"; arr=( ${gpath} ); IFS=$IFS_s

# -- Traitement pour 'Resultats Editique'
if ( [[ "${arr[1]}" == "Resultats Editique" ]] ) then
	log 0 "${PROC}" "Resultats Editique - Surcharge du Path Destination"
	HEAD=${gpath//\//#}
	if ( [[ ! -z "${fic0}" ]] ) then
		DEST_path="/Resultats Editique/sasrecv/$HEAD#$fic0"
	else
		DEST_path="/Resultats Editique/sasrecv/$HEAD#$fic"
	fi
fi
# -- Traitement pour 'Resultats Editique'
