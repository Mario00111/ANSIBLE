...
#
#-------------------------------------------------------------------------------
#	Shell logafpimp_gen.sh appelé par le service KSL lancement_Shell_GRA_suite
#
#
#	yann kloniecki
...
FILENAME=$(basename $2)			# imp: on récupère ex: EDIEPV.04182659.pdf
impedit=${FILENAME%.*}			# nom long du fichier
imp_01=${12} 				#
imp=`echo $imp_01 | awk -F"." '{print $1}' `		# on enlève l'extension ex: EDIEPV.04182659 
...
rsh $PILSERV -l $PILUSER "echo \"$ligne\" > $PILFILE/${nomfic}_${$}.IMP_02"

rsh $PILSERV -l $PILUSER "echo \"$ligne\" > $PILFILE/${nomfic}_${$}.IMP_03"

echo "----------------------------------------------------------" >> $LOG
echo "     logafpimp_gen.sh     ${nomfic}_${$}.IMP_02" >> $LOG
echo "     logafpimp_gen.sh     ${nomfic}_${$}.IMP_02" >> $LOG
echo "----------------------------------------------------------" >> $LOG
